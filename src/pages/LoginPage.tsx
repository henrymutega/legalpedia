  ...
  useEffect(() => {
    ...
    if (params.get('verified') === '1') {
      ...
      supabase.auth.signOut().finally(() => {
        toast({
          title: t('auth.email_verified', 'Email verified'),
          description: t('auth.email_verified_desc', 'Your email has been verified. Please sign in to continue.'),
        });
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      if (data.user && data.user.identities && data.user.identities.length === 0) {
        setError(t('auth.email_registered_desc', 'This email is already registered. Please sign in instead.'));
        setIsLogin(true);
        setLoading(false);
        return;
      }
        ...
        <div className="w-full max-w-md bg-card rounded-lg shadow-card p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="font-heading text-2xl font-bold text-foreground">
              {mode === 'forgot'
                ? t('auth.forgot_title', 'Reset Password')
                : isLogin
          ...
          {mode === 'forgot' ? (
            resetSent ? (
              <div className="text-center space-y-4">
                <div className="p-4 rounded-md bg-gold/10 text-foreground text-sm">
                  <p className="font-semibold mb-1">{t('auth.reset_link_sent', 'Check your email')}</p>
                  <p className="text-muted-foreground">{t('auth.reset_link_sent_desc', "If an account exists, we've sent a password reset link.")}</p>
                </div>
                <button
                  onClick={() => { setMode('auth'); setResetSent(false); setError(''); }}
              ...
              <form onSubmit={handleForgotPassword} className="space-y-4">
                ...
                >
                  {loading ? t('auth.sending_reset', 'Sending...') : t('auth.send_reset_link', 'Send reset link')}
                </button>
                <button
                  type="button"
                  onClick={() => { setMode('auth'); setError(''); }}
                  className="w-full text-center text-gold hover:underline text-sm"
                >
                  {t('auth.back_to_login', 'Back to sign in')}
                </button>
              </form>
